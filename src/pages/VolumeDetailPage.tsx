...
function VolumeDetailPage() {
  ...
  // Top 3 SHP + top 3 non-SHP by appearances in this volume. Must be declared
  // before any early returns so hook order is stable across renders.
  const { topSHP, topOther } = useMemo(() => {
    const start = volumeStartChapter ?? 0
    const end = volumeEndChapter ?? 9999
    const ranked = characters
      .map((c) => {
        const apps = c.chapter_list
          ? c.chapter_list.filter((ch) => ch >= start && ch <= end).length
          : 0
        return { ...c, volumeAppearances: apps }
      })
      .filter((c) => c.volumeAppearances > 0)
      .sort((a, b) => b.volumeAppearances - a.volumeAppearances)

    return {
      topSHP: ranked.filter((c) => STRAW_HAT_IDS.has(c.id)).slice(0, 3),
      topOther: ranked.filter((c) => !STRAW_HAT_IDS.has(c.id)).slice(0, 3),
    }
  }, [characters, volumeStartChapter, volumeEndChapter])
                        ...
                        <div className="flex gap-3">
                          {topSHP.map((c) => (
                            <CharacterPortrait
                              key={c.id}
                              character={c}
                              appearances={c.volumeAppearances}
                            />
                          ))}
                        </div>
                      </div>
