...
async function fetchChaptersByArc(
  startChapter: number,
  endChapter: number
): Promise<Chapter[]> {
  try {
...
function Tag({ children, to }: { children: React.ReactNode; to?: string }) {
  const className =
    'inline-block px-3 py-1 rounded-full text-sm font-medium transition-colors bg-purple-50 text-purple-700 hover:bg-purple-100'
...
function ArcDetailPage() {
  ...
  const nextArc =
    currentIndex < sortedArcs.length - 1 ? sortedArcs[currentIndex + 1] : null
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                previousArc && navigate(`/arcs/${previousArc.arc_id}`)
              }
