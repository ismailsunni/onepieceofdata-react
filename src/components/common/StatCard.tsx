...
type Color = 'blue' | 'green' | 'purple' | 'amber' | 'pink' | 'emerald'
...
const colorStyles: Record<Color, { bg: string; border: string; icon: string; iconBg: string; text: string }> = {
  blue:    { bg: 'bg-blue-50',    border: 'border-blue-200',    icon: 'text-blue-600',    iconBg: 'bg-blue-100',    text: 'text-blue-900'    },
  green:   { bg: 'bg-green-50',   border: 'border-green-200',   icon: 'text-green-600',   iconBg: 'bg-green-100',   text: 'text-green-900'   },
  purple:  { bg: 'bg-purple-50',  border: 'border-purple-200',  icon: 'text-purple-600',  iconBg: 'bg-purple-100',  text: 'text-purple-900'  },
  amber:   { bg: 'bg-amber-50',   border: 'border-amber-200',   icon: 'text-amber-600',   iconBg: 'bg-amber-100',   text: 'text-amber-900'   },
  pink:    { bg: 'bg-pink-50',    border: 'border-pink-200',    icon: 'text-pink-600',    iconBg: 'bg-pink-100',    text: 'text-pink-900'    },
...
function StatCard({
  ...
  if (variant === 'simple') {
    ...
    if (link && !loading) {
      return (
        <Link
          to={link}
          className={`bg-white border border-gray-200 rounded-xl p-5 text-center hover:border-gray-300 hover:shadow-sm transition-all duration-200 block group ${className}`}
        >
          {simpleContent}
        </Link>
      )
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-xl p-5 text-center ${className}`}>
        {simpleContent}
      </div>
    )
  }

  // --- Enhanced variant (analytics style) ---
  ...
  const cardClasses = `${styles.bg} ${styles.border} border rounded-xl p-6 transition-all duration-200 ${className} relative`
