...
describe('fetchDatabaseStats', () => {
  ...
  it('returns correct counts and computed fields', async () => {
    ...
    vi.mocked(supabase!.from).mockImplementation(() => {
      ...
      // Call 6: num_page select for totalPages
      if (callIndex === 6) return buildPagesChain([{ num_page: 20 }, { num_page: 30 }]) as never
      // Call 7: first chapter date
      if (callIndex === 7) return buildDataChain({ date: '1997-07-19' }) as never
      // Call 8: last chapter date
